...
function PresentationSlideList(props: Slide) {
    ...
    const workSlide = ElementsList(props.elementsList)
    return (
...
function DrawingTextElementList(props:TextElement) {
    let Bold = "unset"
    let Underlined = "none"
    let Italic = "unset"
    if (props.bold)
    ...
    return (
        <div id={props.id} style={WorkTextStyle} className={styles.zoom}>
            <p style={WorkText}>{props.text}</p>
        </div>
    )
}

function DrawingPictureElementList(props:PictureElement) {
    const PictureStyle = {
        width: props.size.width,
        height: props.size.height
    }
    ...
    return (
        <div id={props.id} className={styles.zoom} style={Picture}>
           <img src={props.src} style={PictureStyle}></img>
        </div>
    )
}

function DrawingCircleList(props:Circle) {
    const CircleStyle = {
        left: props.startingPoint.x,
        top: props.startingPoint.y,
        width: props.size.width + 2*props.borderWidth,
    ...
    return (
        <svg className={styles.zoom} style={CircleStyle}>
            <ellipse rx={props.size.width/2 + props.borderWidth/2} ry={props.size.height/2 + props.borderWidth/2} cx={props.size.width/2 + props.borderWidth} cy={props.size.height/2 + props.borderWidth}
                    stroke={props.borderColor}
                    fill={props.fillColor}
                    strokeWidth={props.borderWidth}
...
function DrawingTriangleList(props:Triangle) {
    const TriangleStyle = {
        left: props.startingPoint.x,
        top: props.startingPoint.y,
        width: props.size.width + 2*props.borderWidth,
    ...
    const Points:string = `${x1},${y1} ${x2},${y2} ${x3},${y3}`;
    return (
        <svg className={styles.zoom} style={TriangleStyle}>
            <polygon points={Points}                     
                    stroke={props.borderColor}
                    fill={props.fillColor}
                    strokeWidth={props.borderWidth}/>
...
function DrawingRectangleList(props:Rectangle) {
    const RectangleStyle = {
        left: props.startingPoint.x,
        top: props.startingPoint.y,
        width: props.size.width + 2*props.borderWidth,
    ...
    return (
        <svg className={styles.zoom} style={RectangleStyle}>
            <rect
                width={props.size.width}
                height={props.size.height}
                x={props.borderWidth/2}
...
function ElementsList(elementsList: Array<TextElement|PictureElement|Circle|Rectangle|Triangle>)
{
    let ElementsArray:Array<JSX.Element> = []
    if (elementsList)
    {
